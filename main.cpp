...
namespace offsets {
    constexpr auto dwLocalPlayer = 0xDEB99C;
    constexpr auto dwEntityList = 0x4E0102C;
    constexpr auto dwViewMatrix = 0x4DF1E74;
...
INT APIENTRY WinMain(HINSTANCE instance, HINSTANCE, PSTR, INT cmd_show) {
    ...
    while (running) {
        ...
        const auto local_player = memory::read<DWORD>(handle, client + offsets::dwLocalPlayer);

        if (local_player) {
            const auto local_team = memory::read<int>(handle, local_player + offsets::team_num);
            const auto view_matrix = memory::read<ViewMatrix>(handle, client + offsets::dwViewMatrix);

            for (int i = 1; i < 32; ++i) {
                const auto player = memory::read<DWORD>(handle, client + offsets::dwEntityList + i * 0x10);
                ...
                const auto bones = memory::read<DWORD>(handle, player + offsets::m_dwBoneMatrix);
